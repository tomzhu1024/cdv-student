...
function randomKey(length) {
    ...
    if (previousKeys.filter(str => str.length === length).length === charactersLength ** length) {
        return randomKey(length + 1);
    }
    let result;
    while (true) {
        result = '';
        ...
        if (!previousKeys.hasOwnProperty(result)) {
            previousKeys.push(result);
            break;
        }
    }
    return result;
...
console.log("testing anti-repetition. previous keys:", previousKeys);
console.log("testing anti-repetition. random key:", randomKey(1));
